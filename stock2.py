...
#		 plt.show()
	...
	def draw_screen(self):
		self.ax.cla()
		title = " Avg. Reward: %f Reward: %f Total Test: %d" % (
						self.total_reward,
						self.latest_reward,
						self.total_test)
	...
	def _update_stock(self, move):
		...
		if(move == 1):
			if(self.buy_check==False):
				...
				self.buy_point_p = self.current_point
			...
			if(self.buy_check):
				...
				if(self.data[self.current_point][3] > self.data[self.current_point+1][3]):
					...
					#self.current_reward = self.current_reward
				else:
					self.current_reward = 0
				...
				if(self.data[self.current_point][3] < self.data[self.current_point+1][3]):
					#self.time_step = self.time_step + 1
					#self.current_reward -= self._time_reward(self.time_step) + 10*(self.end_price / self.buy_price - 1)
					#self.current_reward -= self._time_reward(self.time_step)
					self.current_reward = 0
				else:
					#self.current_reward = 0
			...
			if(self.buy_check):
				...
				min_val = np.array(self.data[self.buy_point_p:self.current_point])
				#print(min_val)
				#print([min_val[i][2] for i in range(len(min_val))])
				#self.current_reward = 30*(self.start_price/self.buy_price -1.2 + 0.2*np.min([min_val[i][3] for i in range(len(min_val))]) / self.buy_price)
				self.time_step = 0
				self.current_reward = self._cell_reward()
				self.delayed_reward = 10*(self.start_price/self.buy_price -1)
				self.latest_reward = self._cell_reward()
	...
	def _is_gameover(self):
		...
		if (self.current_point >= self.max_day-5):
			print('매도 횟수: %d' % len(self.cell_point))
