...
class DQN:
	...
	def __init__(self, session, day_length, data_kind, n_action):
		...
		# 현재 게임판의 상태
		self.state = None
		self.learning_check = False
		self.learning_reward = 0
		self.hist_cnt = 0
	...
	def get_action(self):
		...
		[action, prob] = [np.argmax(Q_value[0]), np.max(Q_value[0])]
	...
	def remember(self, state, action, reward, terminal):
		...
		if(self.learning_check):
			...
			#	self.memory[i][3] = reward
				#print("%d %f" % (self.memory[i][2], self.memory[i][3]))
				#time.sleep(1)	
	...
	def _sample_memory(self):
		...
		if(len(self.memory) < self.BATCH_SIZE):
			self.Sample_Num = len(self.memory)
		else:
			self.Sample_Num = self.BATCH_SIZE

		sample_memory = random.sample(self.memory, self.Sample_Num)
